...
gulp.task('generate-guestlist', function () {
  ...
  gulp
    .src('tests/rsvpValidation.test.js', { allowEmpty: true })
    // Replace any previous guest list definition (capturing multiline objects)
    .pipe(
      replace(
        /const guestList = {[^]*?};/,
        `const guestList = ${guestListJson};`
      )
    )
  ...
  return (
    gulp
      ...
      .pipe(gulp.dest('js'))
  ); // Write the output back to the 'js' folder
});

// Task to compile all SCSS files to compressed CSS, excluding already minified files
gulp.task('sass', function () {
  return gulp
    .src(['./sass/**/*.scss', '!./sass/**/*.min.scss']) // Source all SCSS files, excluding already minified ones
...
gulp.task('minify-css', function () {
  return gulp
    .src(['./css/**/*.css', '!./css/**/*.min.css']) // Source all CSS files, excluding already minified ones
...
gulp.task('minify-js', function () {
  return gulp
    .src(['./js/**/*.js', '!./js/**/*.min.js']) // Source all JavaScript files, excluding already minified ones
...
gulp.task(
  'default',
  gulp.series('generate-guestlist', 'sass', 'minify-css', 'minify-js')
); // Run all tasks in sequence when 'gulp' is executed without arguments
